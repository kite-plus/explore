...
- **条件请求**：会带上次的 `ETag` 和 `Last-Modified`，没有变化时你的服务器只需回 `304`。
...
- **Conditional requests**: it sends the last `ETag` and `Last-Modified`, so your server can answer `304` when nothing changed.
