...
> 状态：本站账号、订阅流和域名认领已实现；OIDC 待接入 · 最近更新：2026-09-25
...
| 排序 | 发布时间倒序 | 文章评分从高到低 `[待定]` | 发布时间倒序 |
| 范围 | 缓存里的全部（每博客最多 20 篇），按页往下翻 | —— | 缓存里的全部（每博客最多 20 篇） |
| 要登录 | 否 | 否 | 是 |
| 防刷屏 | 同一博客每天最多 3 篇 | —— | 不限 |
| 缓存与收录 | 可缓存、可收录 | 可缓存；评分上线前 `noindex` | `private, no-store`、`noindex` |
| 筛选 | 标签、语言 | —— | 标签、语言 |

最新和订阅按发布时间排序，不使用个人行为数据。推荐会给文章评分，把优质文章排在前面；评分方法 `[待定]`，同样不使用个人行为数据（§0）。评分上线前，推荐页只说明即将上线，并链接到最新。
