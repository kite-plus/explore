...
首页把三条文章流做成同一排标签，筛选放在同一行：“最新”按发布时间展示全部可见文章，“推荐”留给以后的文章评分，“订阅”只含当前用户订阅博客的文章。三页共用页头的“文章”导航项和同一个标题，切换标签时只换说明和文章；页头不再单列“订阅”。博客语言用紧凑切换组，文章标签放进原生 `<details>` 菜单，选择后仍通过链接保留另一项筛选条件。
