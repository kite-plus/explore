...
- **默认不发 JavaScript**：页面在服务端渲染成纯 HTML，爬虫拿到的就是最终内容。基础页面只有主题切换脚本；含文章列表的页面另有一小段按需检测脚本（§4）。
...
端到端测试守住这条规则：公开页面只有主题脚本，文章列表页面再有状态检测脚本；两段内容均与源文件逐字相同（§10）。

首页把文章流入口与筛选放在同一条横向导航上。当前“发现”仍按发布时间展示文章，“订阅”标明尚未开放；未来接入推荐流和订阅流时沿用这个入口。博客语言用紧凑切换组，文章标签放进原生 `<details>` 菜单，选择后仍通过链接保留另一项筛选条件。
...
- `404` 与 `503` 的状态码正确；
- 公开页面只有主题切换脚本，文章列表页面另有状态检测脚本；内容与源文件一致，哈希在 CSP 里；没有内联 `style` 属性、不发 Cookie，并带着 CSP 响应头；
