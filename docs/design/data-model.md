...
> 原则见 [architecture.md §0.1](architecture.md#0-两个核心判断)：只存"有哪些博客"，不存"博客写了什么"。本文把它落到 PostgreSQL 上。
...
V1 **没有**账号表：读者不需要账号；维护者用配置里的访问令牌（[api.md §4](api.md#4-管理接口)），到 E3 做管理界面时再加。

数据库里**永远不出现**的东西：文章正文、HTML、图片；读者的任何信息（账号、Cookie、IP、点击记录）。§6 的 schema 守护会在有人加列时让 CI 失败。
